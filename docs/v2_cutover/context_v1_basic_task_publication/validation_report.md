...
对上一节的 `OAGNET_ASL_RESPONSE_AMBIGUITY_VALIDATION` 做了同请求受控诊断。4 次 Oagent 调用均在收到 ASL 后立即停止，SQL 和数据库调用为 0；其中一次复现了真正波动机制：模型先声称省份字段未映射，随后既有合同修复又依据唯一发布目录映射补入 `dim_province.province_name = 江苏省`，但旧的字段型 ambiguity 没有删除。旧代码只清理正文包含过滤值的 ambiguity，因此 DataAnalysis 在正确 ASL 上仍收到 `ASL_AMBIGUOUS`。

修复归属 Oagent 的 Intent-ASL 合同层：只有调用方过滤合同已唯一映射且完整谓词已存在时，才删除引用同一语义或物理字段的旧 filter ambiguity。其他字段、非唯一映射和通用短字段尾名继续 fail closed。没有修改 V2 Context、Bridge、V1、SQL Translator、Scope、Catalog 或 Redis。

离线全量从基线 **697 passed / 1 existing failed** 变为候选 **701 passed / 1 same existing failed**；4 个新增正反例通过，`old-pass→new-fail=0`。受影响合同和安全集合为 **107 passed / 1 same existing failed**。唯一既有失败 node 在基线与候选完全相同。

仅重启 Oagent 8021 后，以新隔离会话真实调用 8088：

| 轮次 | V2 状态 | Oagent 合同 | SQL / DB | 响应 |
|---|---|---|---|---|
| 查询去年江苏省订单笔数 | Task v1；2025、江苏省、订单笔数 | PASS；同字段旧 ambiguity 已清理 | REACHED / REACHED | HTTP 200 / COMPLETED |
| 换今年 | 同一 Task v2；只把时间 REPLACE 为 2026；completed question=`查询2026年江苏省订单笔数。` | PASS；同字段旧 ambiguity 已清理 | REACHED / REACHED | HTTP 200 / COMPLETED |

Redis 只读复核为 state version 2、一个 Task、versions `[1,2]`；m1/m2 均为 `V2_RESOLVED_COMPLETED_QUESTION` 和 `V1_EXECUTION_RESPONSE_SAVED`。Oagent 新 PID 为 28896；DataAnalysis 35348、SQL Translator 26172 未重启。两轮均有新的 `oagnet-asl` 与 `data-source:58` 证据，未复用首轮查询结果。

`FIRST_FAILURE_STAGE = CLOSED_AT_OAGENT_CONTRACT_REPAIR`

详细方案和安全负例见 [asl_ambiguity_solution.md](asl_ambiguity_solution.md)。
