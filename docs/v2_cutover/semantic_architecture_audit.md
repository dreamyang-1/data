...
| Entity values | source_value_recognition.py and Oagnet catalog_value_sources.py; bounded exact read with current scope and mapping proof | Private exact and bounded-probe fixtures now replay native binding/finish. Probe requires one selected field and at most64 distinct short values; the city field exceeds this limit. Field/value semantic accuracy and complete retrieval ranking remain unproved. |
