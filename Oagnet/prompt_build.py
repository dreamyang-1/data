...
- 上游时间或限制为 null 仅表示未显式提取，不代表禁止应用 DSL 已声明且适用的默认值。DSL 没有定义时，不自行补最近一年、1000条或其他经验值。
...
- 用户未要求时间时，先检查所选对象的 DSL 是否有适用的默认时间规则；有则按该规则生成可执行范围，无则 time_context=null。不要把指标名称中的“年度”或无关示例当作时间要求；通用提示词不自行指定默认月份数。
