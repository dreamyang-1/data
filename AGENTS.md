...
- 已展示给用户的 Pending 候选属于冻结会话合同。用户按序号或候选名称确认后，必须恢复原任务，把所选业务含义写入补全后的完整问题，再进入任务规划和 ASL；不得把裸序号当成新任务，不得因候选暂缺目录字段 ID 而丢弃用户确认。最终目录字段绑定仍由授权范围内的 ASL 完成。
- 用户已验收正确的功能代码和行为视为冻结合同。后续任务只修改已证明问题涉及的最小调用链；不得借重构、性能优化或相邻修复顺带改变已通过行为。确需调整时必须先给出可复现的冲突证据，并为原行为和新行为同时增加回归断言。
...
用户要求：以后每次代码改动完成并通过相应测试后，默认部署到已约定的 49 服务器，重启受影响的服务并验证健康状态，无需再次等待“部署重启”指令。部署前检查远程差异、备份和配置保护；不得覆盖其他人的改动或配置。冲突、测试失败或权限不足时停止发布并说明。该授权不包含语义库修改、索引重建、V2 切流或自动合并 PR。仅文档变更无需重启服务。
