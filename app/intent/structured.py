...
31. dimensions 只表示最终结果的展示或分组粒度。只有“按某字段统计/分组/汇总/拆分”“各个/每个/分别/各自”等明确要求逐值展开的表达，才把该字段放入 dimensions；字段在问题中出现，不等于用户要求按它分组。
32. 具体实体值用于限定查询范围，不得作为 dimensions。例如“上海市的经销商名单”中上海市是城市筛选值，经销商是返回对象；“费森尤斯产品的销售额”中费森尤斯是厂家或品牌筛选值，不得因此加入厂家、品牌或产品维度。
33. “名单、清单、明细、输出某对象列表”首先描述 DETAIL_QUERY 的返回对象和 fields，不自动代表聚合维度；只有查询同时包含可计算指标且要求每个对象各出一行时，该对象才作为聚合维度。
34. 同一语义字段可以同时承担筛选和维度，但必须同时存在独立证据。例如“只看上海市，按城市统计销售额”中城市既限定为上海市又要求按城市分组；只有等于、包含、大于、小于、不等于、排除等条件而没有展开要求时，只能视为筛选。
35. 时间范围（近30天、起止日期）只限定查询时间，不加入 dimensions；按日、周、月、季度、年展开时才形成时间粒度。时间范围和时间粒度可以同时存在。
36. 多个枚举值如果只限定范围，仍然只是筛选；出现“分别、各自、逐个、对比”并要求每个值独立出结果时，才同时形成维度或比较轴。
37. 语义模糊时不得制造空值筛选，也不得凭空增加分组。只有不同解释会实质改变结果粒度且无法从上下文唯一恢复时，才降低 confidence、写入 ambiguities 并触发澄清。
...
class HybridIntentClassifier:
    ...
    def _supported_entity_category(
        ...
        evidence = {
            ...
            "供应商": "供应商" in compact,
            "医院": bool(re.search(
                r"(?:查询|查看|列出|展示|返回|各个|各|每一个|每个|每家|按)"
                r"(?:[^，,。；;？?]{0,12})医院(?!等级|级别|覆盖率|数量|家数)"
                r"|医院(?:名单|清单|列表|明细|名称|地址|等级|级别|数量|家数|有哪些)",
                compact,
            )),
            "客户": "客户" in compact,
            "门店": "门店" in compact,
