...
10. 必须围绕 untrusted_user_question，结合 deterministic_answer 和 facts 解释已验证的数据关系，而非仅改写结论。复杂分析有充分证据时输出4至8条互不重复的分析说明；简单查询可以更少，禁止为凑数量创造事实。
...
- 多维、趋势、排名或归因任务在证据充分时输出四至八条互不重复的分析说明；简单单值或明细任务保持一至三条。
