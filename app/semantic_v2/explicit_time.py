...
def normalize_initial_assignment(session, parse, edit, value, metrics, now):
    ...
    normalized = [normalize_range(mention.surface,now) for mention in ranges]
    ...
    if explicit_field:
        ...
        if anchor.semantic_role != 'TIME_FIELD':
            raise RecognitionFailure('V2_SLOT_ROLE_CONFLICT')
        reason = 'CURRENT_RANGE_NORMALIZED_WITH_EXPLICIT_TIME_FIELD'
    else:
    ...
    actual = m.TimeSpec(anchor=anchor,range=normalized[0],grain=normalize_grain(grains),
